...
class SearchService:
    async def create_search(self, db: DbSession, search_data: SearchCreate) -> Optional[Search]:
        """
    ...
    async def get_search_by_id(self, db: DbSession, search_id: uuid.UUID) -> Optional[Search]:
        """
        ...
        try:
            result = await db.execute(
                select(Search).filter(Search.id == search_id)
            )
    ...
    async def list_searches_by_investigation(self, db: DbSession, investigation_id: uuid.UUID) -> List[Search]:
        """
